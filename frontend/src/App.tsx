  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: 'var(--text-muted)', marginBottom: 4 }}>
        <span>? 速度 (0)</span><span>?? 平衡 (0.5)</span><span>?? 质量 (1)</span>
      </div>
...
function SettingsPanel({ providers, ratio, setRatio, orchThinking, setOrchThinking, agentThinking, setAgentThinking, modelId, setModelId, agentModelMap, setAgentModelMap, visible, onClose }: {
  ...
  return (
    <div className="settings-drawer">
      ...
      <div className="settings-section">
          <div className="settings-section-title">宏观调控模型 思考强度（=严格程度）</div>
      ...
      <div className="settings-section">
        ...
        <div style={{ fontSize: 10, color: "var(--text-muted)", marginTop: 4 }}>
              {agentThinking === "auto" ? "由宏观大模型根据任务复杂度自动决定子代理的严格程度" : `子代理使用 ${agentThinking} 思考强度`}
        </div>
      </div>
      <div className="settings-section">
              ...
              <div key={p.id} className="card">
                ...
                {p.models.map(m => (
                  <div key={m.id} style={{ padding: '3px 0', fontSize: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    {(m.type === "tts" || m.type === "stt") && <span className="badge badge-accent" style={{fontSize:9}}>??? 语音</span>}{(m.capabilities as any).audioScore > 0 && <span className="badge badge-orange" style={{fontSize:9}}>?? 音频 {(m.capabilities as any).audioScore}/10</span>}
                  </div>
                ))}
              </div>
            );
        ...
        {allModels.map(m => (
          <div key={m.id} className="card" style={{ padding: 10, cursor: 'pointer' }} onClick={() => setSelected(m.id)}>
            ...
            <div style={{ display: 'flex', gap: 6, fontSize: 10, flexWrap: 'wrap' }}>
              {(m.capabilities as any).visionScore > 0 && <span className="badge badge-purple" style={{fontSize:9}}>??? 视觉</span>}
      ...
      {reports.map((r, i) => (
        <div key={i} className="card">
          <div className="card-title">
            ...
            {(r.capabilities?.visionScore || 0) > 0 && <span className="badge badge-purple" style={{marginLeft:4}}>??? 视觉</span>}
        ...
        for (const line of parts) {
          if (line.startsWith('event: ')) { evt = line.slice(7).trim(); }
            ...
            try {
              ...
              if (evt === 'status') {
                const msg = d.status === 'thinking' ? '?? 思索中... (' + (d.model||'') + ')' : d.status === 'calling_llm' ? '? 调用模型...' : d.status === 'executing' ? '?? 执行命令 (' + d.commandCount + '个)...' : d.status === 'fixing' ? '?? 修复错误 (尝试' + d.attempt + ')...' : '? ' + d.status;
          ...
          {threads.map(t => (
            <div key={t.id}
              ...
              <span className="thread-actions" style={{ display: 'none', flexShrink: 0 }}>
                <span onClick={(e) => { e.stopPropagation(); setEditingThreadTitle(t.id); }} style={{ cursor: 'pointer', fontSize: 12, marginRight: 2 }} title="编辑标题">??</span>
          ...
          <div className="chat-container">
            ...
            <div className="prompt-bar">
              ...
              <div className="prompt-meta">
                ...
                <span className="prompt-meta-chip" onClick={() => setSettingsOpen(!settingsOpen)}>{orchThinking==='auto'?'自动':orchThinking==='low'?'低(宽松)':orchThinking==='medium'?'中(标准)':'高(严格)'} | {agentThinking==='auto'?'自动':agentThinking==='low'?'低(宽松)':agentThinking==='medium'?'中(标准)':'高(严格)'}</span>
                <span className="prompt-meta-chip" onClick={() => setSettingsOpen(!settingsOpen)}>{ratio<=0.2?'? 速度':ratio>=0.8?'?? 质量':'?? 平衡'} {ratio}</span>
